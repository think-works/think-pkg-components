...
export function updateScrollElementStyle(
  ...
  if (horizontalElement) {
    const offsetX =
      scrollLeft *
      computeRatio(scrollWidth, clientWidth, gapX, minThumbSize).ratio;
    (horizontalElement.firstChild as HTMLDivElement).style.transform =
  ...
  if (verticalElement) {
    const offsetY =
      scrollTop *
      computeRatio(scrollHeight, clientHeight, gapY, minThumbSize).ratio;
    (verticalElement.firstChild as HTMLDivElement).style.transform =
